...
const VISIBILITY_OPTIONS = [
  {
    value: 'PUBLIC',
    label: 'Öffentlich 🌐',
    desc:  'Alle auf BANGME können zuschauen, auch Nicht-Abonnenten',
    badge: 'bg-green-500/20 text-green-400 border-green-500/30',
  },
  {
    value: 'FOLLOWERS',
    label: 'Nur Follower 👥',
    desc:  'Nur Nutzer, die dir folgen, können dem Livestream beitreten',
    badge: 'bg-blue-500/20 text-blue-400 border-blue-500/30',
  },
  {
    value: 'SUBSCRIBERS',
    label: 'Nur Abonnenten 💳',
    desc:  'Nur zahlende Abonnenten, maximale Exklusivität',
...
function GoLiveModal({ onClose }: { onClose: () => void }) {
  ...
  async function handleStart() {
    if (!title.trim()) { setError('Bitte gib einen Titel für deinen Livestream ein'); return }
    setLoading(true); setError(null)
    try {
      const { data } = await api.post('/live/start', { title: title.trim(), visibility })
      const { stream, agoraToken, channelId, agoraUid } = data.data
      ...
      router.push(`/live/broadcast?${params.toString()}`)
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message
      setError(msg ?? 'Livestream konnte nicht gestartet werden. Bitte versuche es erneut.')
      setLoading(false)
    }
  }
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-brand-border">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-white font-black text-lg">Live gehen</h2>
          </div>
        ...
        <div className="px-6 py-6 space-y-5">
          {/* Title */}
          <div>
            <label className="block text-white font-semibold text-sm mb-2">Stream-Titel <span className="text-brand-red">*</span></label>
            <input
              type="text"
              value={title}
              onChange={e => setTitle(e.target.value)}
              placeholder="z. B. Exklusives Q&A mit mir"
              maxLength={100}
          ...
          {/* Visibility picker */}
          <div>
            <label className="block text-white font-semibold text-sm mb-2">Wer kann zuschauen?</label>
            <div className="space-y-2.5">
              {VISIBILITY_OPTIONS.map(opt => (
                ...
                >
                  ...
                  {visibility === opt.value && (
                    <span className={`ml-auto shrink-0 px-2 py-0.5 rounded-full border text-xs font-bold ${selected.badge}`}>
                      Ausgewählt
                    </span>
                  )}
                </button>
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {loading ? (
                <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Wird gestartet...</>
              ) : (
                <>Livestream starten</>
              )}
            </button>
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button className="bg-brand-surface border border-brand-border text-brand-text px-4 py-2 rounded-lg text-sm font-bold">Massen-Nachricht</button>
          <button
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Einnahmen (Monat)', value: '$1,842', trend: '▲ 12%', up: true },
          { label: 'Abonnenten', value: '1,254',    trend: '▲ 4%',  up: true },
          { label: 'Follower',   value: '9,816',    trend: '▲ 2%',  up: true },
          { label: 'Nachrichten (7 T.)', value: '482',    trend: '▼ 3%',  up: false },
        ].map(kpi => (
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="col-span-2 bg-brand-surface border border-brand-border rounded-xl p-5">
          <h3 className="text-white font-bold mb-4">Einnahmenaufschlüsselung</h3>
          <div className="grid grid-cols-2 gap-3">
            {[['Trinkgeld','$426'],['Content-Verkäufe','$1,098'],['Abonnements','$278'],['Sonstiges','$40']].map(([label, val]) => (
        ...
        <div className="bg-brand-surface border border-brand-border rounded-xl p-5">
          <h3 className="text-white font-bold mb-3">Schnellaktionen</h3>
          <div className="space-y-2">
            {['Content hochladen','Profil bearbeiten','Creator-Preise','Promos','Abrechnung & Auszahlungen'].map(action => (
      ...
      {/* Financial table */}
      <div className="bg-brand-surface border border-brand-border rounded-xl p-5">
        <h3 className="text-white font-bold mb-4">Finanzcenter</h3>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-brand-border">
              {['Datum','Typ','Brutto','Plattformgebühr (25%)','Netto','Notiz'].map(h => (
          ...
          <tbody>
            {[
              ['2025-09-01','Abonnement','$20.00','$5.00','$15.00','Monatsabo - @NovaRush'],
              ['2025-09-02','Trinkgeld','$10.00','$2.50','$7.50','Trinkgeld - @LunaVelvet'],
              ['2025-09-03','Verkauf','$15.00','$3.75','$11.25','Premium-Foto'],
            ].map((row, i) => (
